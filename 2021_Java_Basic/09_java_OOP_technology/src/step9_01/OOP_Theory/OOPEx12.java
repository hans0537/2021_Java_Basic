...
class Music extends Thread{
	...
	// Thread 클래스로 부터 제공되는 run() 메소드 이며
...
public class OOPEx12 {
	...
	public static void main(String[] args) {
		...
		// Thread클래스를 상속받을 경우 객체에서 start() 메소드를 이용하여
		// 스레드를 시작한다. -> 클래스 안에 오버라이딩된 run() 메소드 호출
		musicPlay.start();
		
		// Runnable interface 를 구현할경우 Thread 객체에서 생성자의 인수로 
		// Runnable interface 를 구현한 객체를 주입하여 새로운 객체를 만들고
		// 새로운 객체를 통해 .start()메소드를 이용하여 스레드를 시작한다 -> run() 메소드 호출
